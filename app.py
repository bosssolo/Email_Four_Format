...
# Remove protocol (http, https), www, paths, ports, query params, fragments, and trailing dots
    ...
    # If input looks like a URL, extract netloc; else, treat as domain
    if "://" in domain:
        parsed = urlparse(domain)
        domain = parsed.netloc or parsed.path
    else:
        # Remove protocol if user typed it without slashes (e.g., "http:company.com")
        domain = re.sub(r'^(https?:)?//?', '', domain)
    
    # Remove 'www.' if present
    domain = re.sub(r'^www\.', '', domain)
    # Remove any path, port, query, or fragment
